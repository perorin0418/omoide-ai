...
class Synchronizer:
    ...
    def synchronize(self, changed_paths: list[Path] | None = None, *, force_graph: bool = False) -> dict[str, list[str]]:
        ...
        for path in sorted(relevant):
            ...
            existing_manifest = manifest.get(path_key, {})
            document_unchanged = existing_manifest.get("document_hash") == document_hash
            if document_unchanged and not force_graph:
                continue

            record = self.store.load_path(path)
            if not document_unchanged:
                chunks = chunk_markdown(path, record.memory_id, text)
                existing_chunks: dict[str, str] = existing_manifest.get("chunks", {})
